		...
		button.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				...
				if(textPane.getText().trim().toString().length() == 0||
					...
					JOptionPane.showMessageDialog(null,"Id与名称不能为空","操作失败!",JOptionPane.ERROR_MESSAGE);
			    	i = 1;					
				}
				if(i == 0){
					wh.setWarh_id(textPane.getText().trim().toString());
					wh.setWarh_name(textPane_1.getText().trim().toString());
					wh.setWarh_remark(textPane_2.getText().trim().toString());
