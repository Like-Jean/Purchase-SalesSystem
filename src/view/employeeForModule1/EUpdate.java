...
public class EUpdate {
	...
	private void initialize() {
		...
		JButton button = new JButton("确认修改");//确认
		button.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				
				epl.setEmpl_department(textPane_2.getText().trim());
				epl.setEmpl_name(textPane_1.getText().trim());
				epl.setEmpl_phoneNumber(textPane_5.getText().trim());
				epl.setEmpl_position(Integer.parseInt(textPane_3.getText().trim()));
				epl.setEmpl_salary(Float.parseFloat(textPane_4.getText().trim()));
